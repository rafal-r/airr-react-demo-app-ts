...
import Welcome, { viewName as WelcomeViewName } from "./views/Welcome";
import MayersView, { viewName as MayersViewName } from "./views/Mayers";
import SidepanelView, {
    viewName as SidepanelViewName
} from "./views/Sidepanel";
import ScenesView, { viewName as ScenesViewName } from "./views/Scene";
import TabsView, { viewName as TabsViewName } from "./views/Tabs";
import update from "immutability-helper";

export const viewName = "main-scene";

const sidepanelSide: Placement = "left";
const sidepanelSizeFactor = 0.66;

export interface MainSceneProps extends SceneProps {
    handleViewportViewPop: () => void;
    handleViewportScenePush: (sceneConfig: ViewConfig<SceneProps>) => void;
}

export default class MainScene extends Scene<MainSceneProps> {
    constructor(props: MainSceneProps) {
        ...
        this.state = {
            ...
            handleBackButton: this.customHandleBackButton,
            mayers: [],
            sidepanel: {
                type: Sidepanel,
                props: {
    ...
    getSidepanelContent = (viewname: string) => {
        let description;

        if (
            viewname === MayersViewName ||
            viewname === SidepanelViewName ||
            viewname === ScenesViewName ||
            viewname === TabsViewName ||
            viewname === WelcomeViewName
        ) {
            description = this.viewsConfig[viewname].props.description;
        }
    ...
    handleSidepanelSizeChange = (e: SyntheticEvent<HTMLElement>) => {
        const sizeFactor = parseFloat(e.currentTarget.dataset.value || "");
    ...
    handleSidepanelSideChange = (e: SyntheticEvent<HTMLElement>) => {
    ...
    handleMayerOpen = (e: SyntheticEvent<HTMLElement>, cfg: MayerProps) => {
        const config: MayerProps = {
            ...cfg,
            buttons: [
                {
                    children: "Ok",
    ...
    handleSceneViewMenuClick = (viewConfig: ViewConfig<ViewProps>) => {
        return this.changeView(viewConfig);
    };

    handleWelcomeViewMenuClick = (e: SyntheticEvent<HTMLElement>) => {
        const viewName = e.currentTarget.dataset.view;
        if (viewName && viewName in this.viewsConfig) {
    ...
    customHandleBackButton = () => {
        if (this.state.views && this.state.views.length > 1) {
    ...
    handleInfoBtnClick = (content: React.ReactNode) => {
        const config: MayerProps = {
